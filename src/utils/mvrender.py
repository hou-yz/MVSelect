# this is borrowed from https://github.com/ajhamdi/mvtorch/blob/main/mvtorch/mvrenderer.py

from mvtorch.utils import *
from mvtorch.ops import check_and_correct_rotation_matrix
...
class MVRenderer(nn.Module):
    ...
    def __init__(self, nb_views, image_size=224, pc_rendering=True, object_color="white", background_color="black",
                 faces_per_pixel=1, points_radius=0.006, points_per_pixel=1, light_direction="random",
                 cull_backfaces=False, return_mapping=True):
        super().__init__()
    ...
    def render_meshes(self, meshes, color, azim, elev, dist, lights, background_color=(1.0, 1.0, 1.0), ):
        ...
        new_meshes = Meshes(verts=verts,
                            faces=faces,
                            textures=None)
        max_vert = new_meshes.verts_padded().shape[1]

        new_meshes.textures = Textures(verts_rgb=color.cuda() * torch.ones((c_batch_size, max_vert, 3)).cuda())
        ...
        camera = OpenGLPerspectiveCameras(device="cuda:{}".format(torch.cuda.current_device()),
                                          R=R[None, 0, ...], T=T[None, 0, ...])
        ...
        rendered_images = unbatch_tensor(rendered_images, batch_size=self.nb_views,
                                         dim=1, unsqueeze=True).transpose(0, 1)
    ...
    def render_points(self, points, color, azim, elev, dist, background_color=(0.0, 0.0, 0.0), ):
        point_cloud = Pointclouds(points=points.float(),
                                  features=color.to(points.device) * torch.ones_like(points, dtype=torch.float)).cuda()
        ...
        raster_settings = PointsRasterizationSettings(image_size=self.image_size,
        ...
        rendered_images = compositor(fragments.idx.long().permute(0, 3, 1, 2),
                                     weights, point_cloud.features_packed().permute(1, 0), )
        # permute so image comes at the end
        rendered_images = rendered_images.permute(0, 2, 3, 1)

        rendered_images = unbatch_tensor(rendered_images, batch_size=self.nb_views,
                                         dim=1, unsqueeze=True).transpose(0, 1)
        weights = unbatch_tensor(weights, batch_size=self.nb_views, dim=1, unsqueeze=True).transpose(0, 1)
        indxs = unbatch_tensor(fragments.idx.long().permute(0, 3, 1, 2), batch_size=self.nb_views,
                               dim=1, unsqueeze=True).transpose(0, 1)
    ...
    def light_direction(self, azim, elev, dist):
        if self.light_direction_type == "fixed":
            ...
            relative_view = Variable(
                camera_position_from_spherical_angles(distance=batch_tensor(dist.T, dim=1, squeeze=True),
                                                      elevation=batch_tensor(elev.T, dim=1, squeeze=True),
                                                      azimuth=batch_tensor(azim.T, dim=1, squeeze=True))).float()
    ...
    def forward(self, meshes, points, azim, elev, dist, color=None):
        ...
        if not self.pc_rendering:
            ...
            rendered_images, cameras = self.render_meshes(meshes=meshes, color=color, azim=azim, elev=elev, dist=dist,
                                                          lights=lights, background_color=background_color)
        else:
            rendered_images, indxs, weights, cameras = self.render_points(points=points, color=color, azim=azim,
                                                                          elev=elev, dist=dist,
