...
def draw_curve(path, x_epoch, train_loss, test_loss, train_result=None, test_result=None):
    fig = plt.figure()
    ...
    if train_result is not None:
        ax2 = fig.add_subplot(122, title="acc")
        ax2.plot(x_epoch, train_result, 'bo-', label='train' + ': {:.1f}'.format(test_result[-1]))
    else:
