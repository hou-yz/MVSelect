...
def main(args):
    ...
    if args.select:
        ...
        pretrained_dict = torch.load(f'{load_dir}/MVCNN.pth')
    ...
    if args.resume:
        pretrained_dict = torch.load(f'logs/{args.dataset}/{args.resume}/MVCNN.pth')
        model_dict = model.state_dict()
        pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict and 'cam_' not in k}
        model_dict.update(pretrained_dict)
        model.load_state_dict(model_dict)
    ...
    if not args.eval:
        ...
        for epoch in tqdm.tqdm(range(1, args.epochs + 1)):
            ...
            if epoch % max(args.epochs // 10, 1) == 0:
                ...
                draw_curve(os.path.join(logdir, 'learning_curve.jpg'), x_epoch,
                           train_loss_s, test_loss_s, train_prec_s, test_prec_s)
...
if __name__ == '__main__':
    # settings
    parser = argparse.ArgumentParser(description='Multiview classifier')
